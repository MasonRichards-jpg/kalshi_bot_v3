...
USE_KELLY = True
MIN_CONFIDENCE = 0.4
MARKET_ORDER_FRACTION = 0.15  # Fraction of orders to send as market (taker) vs limit (maker)
...
class SafeCompounder:
    ...
    def __init__(
        ...
        market_order_fraction: float = MARKET_ORDER_FRACTION,
    ):
        self.client = client
    ...
    async def _place_resting_orders(
        ...
        for opp in opportunities:
            ...
            use_market = random.random() < self.market_order_fraction
            # Market orders fill at the ask; limit orders rest at ask-1¢
            price = opp["lowest_no_ask"] if use_market else opp["our_price"]
            order_label = "MKT" if use_market else "LMT"
            cost = contracts * price * 100  # Convert dollars to cents for cost calculation
            profit = contracts * (1.0 - price) * 100  # Convert dollars to cents for profit calculation
            ...
            if self.dry_run:
                ...
                print(
                    f"  🏷️ [DRY/{order_label}] NO x{contracts} @ ${price:.2f} | "
            ...
            try:
                ...
                if filled > 0:
                    stats["filled"] += filled
                    print(
                        f"  🎯 FILLED [{order_label}] NO x{filled}/{contracts} @ ${price:.2f} | "
                        f"edge:${opp['edge']:.2f} +${filled * (1.0 - price):.2f} | {ticker}",
                        flush=True,
                    )
                else:
                    print(
                        f"  ✅ [{order_label}] NO x{contracts} @ ${price:.2f} | {status} | "
    ...
    def _calculate_position_size(self, opp: Dict, portfolio: int, cash: int) -> int:
        ...
        # Bump up to meet minimum profit target if Kelly/fixed came in too low
